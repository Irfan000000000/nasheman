...
const Employee = () => {
  ...
  const viewEmployee = (employee_id) => {
    axios.get(process.env.REACT_APP_API_BASE_URL+`/view-employee/${employee_id}`)
      .then(response => {
  ...
  const editAdmission = (employee_id) => {
    axios.get(process.env.REACT_APP_API_BASE_URL+`/employee-get/${employee_id}`)
      .then(response => {
        ...
        setFilePreview(employee_image ? process.env.REACT_APP_API_BASE_URL+`/uploads/${employee_image}` : '');
      })
      .catch(error => {
        console.error('Error:', error);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editFormData.hidden_id !== "") {
        ...
        const response = await axios.put(process.env.REACT_APP_API_BASE_URL+`/update-employee/${employee_id_get}`, formData, {
          headers: {
        ...
        const response = await axios.post(process.env.REACT_APP_API_BASE_URL+'/submit', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
  ...
  const deleteEmployee = (employee_id_get) => {
    axios.delete(process.env.REACT_APP_API_BASE_URL+`/delete-employee/${employee_id_get}`)
      .then(response => {
                ...
                {loading ? (
                  ...
                  data.map((employee, index) => (
                    <tr key={index}>
                      ...
                      <td className='text-center'>
                        <button className='btn btn-success btn-sm' onClick={() => editAdmission(employee.id)}><i className="fas fa-edit"></i></button>
                      </td>
                      <td className='text-center'>
                ...
                <div style={{ marginBottom: '20px', textAlign: 'center' }}>
                  {employeeData.employee_image && (
                    <div style={{ marginTop: '10px' }}>
                      <img src={process.env.REACT_APP_API_BASE_URL+`/uploads/${employeeData.employee_image}`} alt="Employee" style={{ width: '80px', borderRadius: '5px' }} />
                    </div>
                  )}
                </div>
