...
const EventModal = ({ event, date, onClose, onSubmit, onDelete }) => {
  ...
  useEffect(() => {
    ...
    if (event) {
      ...
      setStart(event.start ? toLocalDatetimeString(event.start) : toLocalDatetimeString(new Date()));
      setEnd(event.end ? toLocalDatetimeString(event.end) : toLocalDatetimeString(new Date()));
      setAllDay(event.allDay || false);
      setIsPublished(event.isPublished !== undefined ? event.isPublished : true);
    } else if (date) {
  ...
  useEffect(() => {
    if (getClasses.length > 0) {
      ...
      setEditFormData((prevState) => ({
        ...prevState,
        class_id:  Array.isArray(allIds) ? allIds : JSON.parse(allIds), // Ensure arra
        class_name: allNames,
      }));
    }
  }, [getClasses, event]);
