...
import EditFeeVoucher from "./EditFeeVoucher"; // Import the inline edit component
...
function FeeVouchers() {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <h5 style={{ margin: 0 }}>Edit Fee Voucher</h5>
          <button
            onClick={() => setShowEdit(false)}
            style={{
              position: "absolute",
        ...
        >
          <EditFeeVoucher onClose={() => setShowEdit(false)} fetchData={fetchData} />
          </div>
        </div>
      )}
      ...
      <div className="d-flex">
        <div className="col-md-12 p-2">
          ...
          <div className="border p-2">
            <div className="d-flex justify-content-between pb-1">
              <div className="d-flex">
                ...
                <div>
                  ...
                  >
                    <i className="fa fa-eye" aria-hidden="true"></i> View Voucher
                  </button>
                </div>
              </div>
            ...
            <table className="table m-0 table-responsive">
              <thead>
                <tr>
                  <th>Check</th>
                  <th>Invoice#</th>
              ...
              <tbody>
                {displayData.map((voucher, index) => (
                  <tr key={index}>
                    ...
                    <td>{voucher.after_due_date_amount + voucher.arrears + voucher.first_advance_payment}</td>
                    <td>{convertDates(voucher.due_date)}</td>
                    <td
                      style={{
                        color:
                          voucher.status === "unpaid" ? "red" : "green",
                      }}
                    >
                    ...
                    {/* <td className="text-center">
                      <div>
                        <button
                          ...
                          disabled={voucher.status === "paid" || voucher.is_arrear == "arrears"}
                        >
  ...
  <div className="tooltip-container">
    ...
    {(voucher.status === "paid" || voucher.is_arrear == "arrears") && (
                    ...
                    {/* <td className="text-center">
                      <div>
                        <button
                          ...
                          onClick={() => deleteData(voucher.id, voucher.full_name, voucher.class, voucher.section_name, voucher.for_the_month, voucher.register_no)}
                          disabled={voucher.status === "paid" || voucher.is_arrear == "arrears"}
                        >
  ...
  <div className="tooltip-container">
    <button
      className="btn btn-danger btn-sm"
      onClick={() => deleteData(voucher.id, voucher.full_name, voucher.class, voucher.section_name, voucher.for_the_month, voucher.register_no)}
      disabled={voucher.status === "paid" || voucher.is_arrear == "arrears" || JSON.parse(voucher.arrears_not_cleared).length > 0}
    >
      <i className="fas fa-trash-alt"></i>
    </button>
    {(voucher.status === "paid" || voucher.is_arrear == "arrears" || JSON.parse(voucher.arrears_not_cleared).length > 0) && (
  <span className="tooltip-text">
    {voucher.status === "paid" 
    ...
    {/* {console.log(JSON.parse(voucher.arrears_not_cleared).length)} */}
    {/* {(voucher.status === "paid" || voucher.is_arrear == "arrears") && (
      <span className="tooltip-text">
        {voucher.status === "paid" 
          ? "Paid voucher cannot be deleted" 
  ...
  const renderVoucherCopy = (title) => (
    <div className="voucher" style={{ position: "relative" }}>
      ...
      >
        ...
        <div>
          <h5
            style={{ margin: 0, fontSize: "14px", textTransform: "uppercase",  padding:"10px" }}
            className="title"
          >
            {/* {"Sir Syed School " + campus_name} */}
                    ...
                    (voucher, index) =>
                      voucher.status && (
                        <tr key={index}>
                          ...
                          >
                            {voucher.status == "paid"
                              ? formatNumber(voucher.received_payment)
                              : voucher.after_due_data_amount + voucher.arrears}
                          </td>
                          <td
                            style={{
                              border: "1px solid #000",
