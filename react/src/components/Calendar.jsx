...
const Calendar = () => {
 ...
 useEffect(() => {
  ...
  setSelectedEvent({
    id: event.id,
    title: event.title,
    start: event.start ? new Date(event.start) : new Date(), // Keep fallback for start
    end: event.end ? new Date(event.end) : event.start ? new Date(event.start) : new Date(), // Fallback to start date if end is missing
    allDay: event.allDay,
    description: event.extendedProps.description || '',
    color: event.backgroundColor,
    isPublished: event.extendedProps.isPublished || false,
