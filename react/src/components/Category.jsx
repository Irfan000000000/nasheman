...
function Category() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editFormData.hidden_id !== "") {
      ...
      axios.put(process.env.REACT_APP_API_BASE_URL+`/update-category/${category_id}`, editFormData)
        .then(response => {
      ...
      try {
        const response = await fetch(process.env.REACT_APP_API_BASE_URL+'/insert-category', {
          method: 'POST',
          headers: {
  ...
  const editItem = (category_id) => {
    ...
    axios.get(process.env.REACT_APP_API_BASE_URL+`/category-get/${category}`)
      .then(response => {
  ...
  const deleteItem = (category_id) => {
    ...
    axios.delete(process.env.REACT_APP_API_BASE_URL+`/delete-category/${category_id}`)
      .then(response => {
    ...
    <>
      <div className="d-flex">
        <div className='col-md-6 p-2'>
          <h5 className='text-warning bg-primary p-2 card-header border'> <i className='fas fa-gift'></i>Category Form</h5>
  ...
  <div className="d-flex justify-content-between align-items-center">
    <div>
      <i className="fas fa-list"></i> Category List
    </div>
