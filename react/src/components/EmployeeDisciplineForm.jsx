...
function EmployeeDisciplineForm() {
...
const handleActivitySubmit = async (e) => {
  ...
  try {
    ...
    const response = await axios.post(
      process.env.REACT_APP_API_BASE_URL + "/employee-discipline",
      editFormData
    );
  ...
  const handleStudentChange = (selectedOption) => {
    ...
    const edit = (id_get) => {
        axios.get(process.env.REACT_APP_API_BASE_URL+`/get-employee-discipline-data/${id_get}`)
            .then(response => {

              // console.log(response.data.results[0]);
              
                const { id, employee_id, full_name, type_of_incident, employee_post, date_of_incident, action_taken, description} = response.data.results[0];
    ...
    const deleteRow = (id_get) => {
        ...
        if (confirm_delete) {
          axios.get(process.env.REACT_APP_API_BASE_URL+`/employee-discipline-delete/${id_get}/${user.user.campus_id}`)
            .then(response => {
    ...
    const fetchData = (shouldLoad = true) => {
       if (shouldLoad) setLoading(true);
        axios.get(process.env.REACT_APP_API_BASE_URL+"/employee-discipline-list", {
            params: {
                page: currentPage,
                limit: totalItem,
 ...
 const handleKeyDown = (e) => {
      ...
      <div className="d-flex">
        <div className="col-md-5 p-2">
          <h5 className="text-warning bg-primary p-2 card-header border">
            {" "}
          <i class="fas fa-exclamation-triangle"></i> Employee Discipline Form
          </h5>
  ...
  options={getStudents.map((student) => ({
    value: student.id,
    label: student.full_name + " (" + student.employee_post + ")"
  }))}
  value={
    editFormData.student_id
      ? {
          ...
          label:
            getStudents.find(
              (student) => student.id === editFormData.student_id
            )?.full_name + 
            " (" + 
            (getStudents.find(
              (student) => student.id === editFormData.student_id
...
<option value="tardiness">Tardiness/Attendance Issues</option>
<option value="absenteeism">Excessive Absenteeism</option>
<option value="performance">Poor Job Performance</option>
...
<option value="theft">Theft/Misuse of Property</option>
<option value="technology">Misuse of Technology</option>
         ...
         <div className='col-md-7 p-2' >
                    <div className="card-header text-warning bg-primary p-2">
                        <div className="d-flex justify-content-between align-items-center">
                            <div>
                                <i className="fas fa-list"></i> Employee Discipline List
                            </div>
                            <div className="d-flex">
...
export default EmployeeDisciplineForm;
