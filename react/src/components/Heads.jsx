...
function Heads() {
    ...
    const initialState = {
        ...
        status: 'On',
        voucher_type:'multiple_voucher_form',
    ...
    const edit = (id_get) => {
        axios.get(process.env.REACT_APP_API_BASE_URL+`/head-get/${id_get}`)
            .then(response => {
                const { id, head_name, status, voucher_type, checked_status } = response.data.results[0];
                console.log(response.data.results[0]);
                setEditFormData({ ...editFormData,
                    head_name: head_name || '',
                    status: status || '',
                    hidden_id: id || '',
                    checked_status: checked_status || 'not_checked',
                    voucher_type: voucher_type || 'multiple_voucher_form'
                });
            })
            .catch(error => {
        ...
        <>
            <div className="d-flex">
                <div className='col-md-6 p-2'>
                    <h5 className='text-warning bg-primary p-2 card-header border'> <i className="fas fa-check-circle"></i> Heads Form</h5>
                    <form className='border p-3' onSubmit={handleSubmit}>
                        ...
                        <div className="form-group row">
                            <label htmlFor="inputEmail3" className="col-sm-2 col-form-label">Shown.On</label>
                            <div className="col-sm-10">
                            <select
                                    name='fee_head_id'
                                    ...
                                    <option value="multiple_voucher_form">Multiple Voucher Form</option>
                                    <option value="single_voucher_form">Single Voucher Form</option>
                        ...
                        <div className="form-group row">
                            <label htmlFor="inputEmail3" className="col-sm-2 col-form-label">Checked</label>
                            <div className="col-sm-10">
                            <select
                                    name='fee_head_id'
                                    value={editFormData.checked_status}
                                    onChange={(e) => {
                                        setEditFormData({ ...editFormData, checked_status: e.target.value });
                                    }}
                                    className={'form-control'}>
                                    <option value="not_checked">Not Checked</option>
                                    <option value="always_checked">Always Checked</option>
                                </select>
                            </div>
                        </div>
